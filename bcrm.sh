...
declare -r ROOT_DISK=$(lsblk -lpo pkname,mountpoint | grep '\s/$' | awk '{print $1}')
...
disk_setup() { #{{{
    ...
    _create_dests() { #{{{
        ...
        while read -r e; do
            ...
            n=$((n + 1))
            unset sname sfstype name kname fstype uuid partuuid type parttype
        done < <(echo "$plist")
    } #}}}
...
Clone() { #{{{
    ...
    _lvm_setup() { #{{{
        ...
        _(){ #{{{
            local sname fs spid ptype type used mp size lsize lv_size f
            for f in ${!SRCS[@]}; do
...
Main() { #{{{
    ...
    _(){ #{{{
        if [[ -z $VG_SRC_NAME && $HAS_LUKS == true ]]; then
            local luks=$(grep -q "$SRC" < <(dmsetup deps -o devname) | awk '{print $1}' | tr -d ':')
